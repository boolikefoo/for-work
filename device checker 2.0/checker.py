...
def serial_check(serial):
    ...
    if serial in dataset.keys():
        ...
        amount_av =  ''
        print(f'%sСерийрый номер - %s{serial} %s- найден!%s' %(fg(2),fg(3), fg(2), attr(0)))
        ...
        if ar_name != or_name:
            ...
            print(f'%sТекущее поличество по ораклу:%s {amount_set[sn_code]["Количество по ораклу"]}' % (fg(5), attr(0)))
            print(f'%sГотовим к списанию: %s{serial.upper()}%s' % (fg(3),fg(3), attr(0))) 
            

            if amount_set[sn_code]['Количество по ораклу'] > 0:
                if serial in trash_set:
                    print('%sДанный серийный номер уже находится в списке к списанию!!!!!%s' % (fg(1), attr(0)))
                else:
                    listok = [serial, dataset[serial]['Название по аргусу'], dataset[serial]['Код по аргусу']]
                    line = ';'.join(listok)
                    add_line(line)
                    trash_set.update({serial : {dataset[serial]['Название по аргусу'] : dataset[serial]['Код по аргусу']}})
                    amount_set[sn_code]['Количество по ораклу'] -= 1
                    print('Осталось к списанию:', amount_set[sn_code]['Количество по ораклу'])

            elif amount < 1:
                print(f'%sНет едениц для списания.%s' % (fg(1), attr(0)))

    else:
        print(f'%sСерийный номер %s{serial} %s%s - не найден!%s' % (fg(1), fg(3), attr(0), fg(196), attr(0)))


print('%s%sДля завершения программы введите end \n\n\n%s' % (fg(2), attr(1), attr(0)))
